...
he inc -446 if xp > -2075"""
  ...
  val his = ListBuffer[Int]()
  
  def getReg( r : String ) : Int = {
  ...
  def setReg( r : String, n : Int ) : Unit = {
    registers(r) = n
  }
  
  def regValues() : List[Int] = {
    registers.values.toList.sorted
  }
  
  case class Instruction( val reg : String, val op : String, val opAmt : Int, val condReg : String, val condOp : String, val condAmt : Int )
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    val is = lines.map( toInstruction( _ ) )
    Console.println( is )
    
    is.foreach( process(_) )
    
    Console.println( registers )
    Console.println( regValues )
    
    Console.println( his.sorted.last )
  ...
  def process( instr : Instruction ) : Unit = {
  ...
  def calcHi() : Unit = {
    val hi = regValues().last
    his += hi
  }
  
  def eval( instr : Instruction ) : Unit = {
    
    val reg = getReg( instr.reg )
    
    val newValue = instr.op match {
      case "inc" => { reg + instr.opAmt }
      case "dec" => { reg - instr.opAmt }
      case _ => {
        Console.println( "unknown op:" + instr.condOp )
        0
      }
    }
    
    setReg( instr.reg, newValue )
  }
  
  def cond( instr : Instruction ) : Boolean = {
    
    // get the reg
    val condReg = getReg( instr.condReg )
    
    instr.condOp match {
      case ">" => { condReg > instr.condAmt }
      case ">=" => { condReg >= instr.condAmt }
      case "<" => { condReg < instr.condAmt }
      case "<=" => { condReg <= instr.condAmt }
      case "==" => { condReg == instr.condAmt }
