...
wnwzo (37)"""
  ...
  case class Prog( name : String, weight : String, var sum : Int = 0, var children : List[Prog], var parent : Option[Prog] ) {
    override def toString() : String = {
      var res = "(" + name +" "+ weight + " " + sum + " " 
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    // val lines = Common.toLines(example)
    val lines = Common.toLines(data)
    
    val ps = lines.map( toProg( _ ) )
    val mps = toMap(ps)
    buildTree( mps, lines )
    Console.println( mps )
    Console.println( findHead(mps) )
    calcSum( findHead(mps) )
    printTree( findHead(mps) )
    Console.println( findUnbalanced( findHead(mps) ) )
    printBalanceInfo( findUnbalanced( findHead(mps) ) ) 

  }
  
  def printBalanceInfo( prog : Prog ) : Unit = {
    
    val parent = prog.parent.get
    
    for( c <- parent.children ){
      Console.println( c.name +" " + c.sum )
    }
    
    Console.println( prog.name +" "+ prog.weight +" "+ prog.sum )
  ...
  def toProg( line : String ) : Prog = {
    val parts = line.split(' ')
    Prog( parts(0), parts(1), 0, List(), None )
  }
  
  def toMap( progs : List[Prog] ) : Map[String,Prog] = {
    
    val map = scala.collection.mutable.HashMap[String,Prog]()
  ...
  def buildTree( map : Map[String,Prog], lines : List[String] ) : Unit = {
    ...
    for( line <- lines ){
      ...
      if( parts.size > 3 ){
        // we have child to map
        
        // find the parent first
        val parent = map(parts(0))
        
        // go through the children
        for( i <- 3 until parts.size ){
          
          // find child from the map
          val id = parts(i).replace(",", "" )
          val c = map(id)
  ...
  def findHead( map : Map[String,Prog] ) : Prog = {
    
    val ps = map.values.toList
    
    ps.find( !_.parent.isDefined ).get 
    
  }
  
  def printTree( prog : Prog ) : Unit = {
    
    printNode( prog )
    Console.print( "\n" )
    
    // get children
    val cs = prog.children
    printChildren( List(cs) )

    Console.println( "\n" )
  }
  
  def printChildren( clist : List[List[Prog]] ) : Unit = {
    
    if( !clist.isEmpty ){
    
      for( cs <- clist ) {
        Console.print( "[" )
        cs.foreach( printNode( _ ) )
        Console.print( "] " )
      }
      Console.print( "\n" )
    
      val cs = clist.flatten
      val gcs = for( c <- cs ) yield { c.children }
        printChildren( gcs )
    }
    
  }
  
  def printNode( prog : Prog ) : Unit = {
    Console.print( "(" + prog.name +" "+ prog.weight +" "+ prog.sum  +")" )
  }
  
  def weightToInt( prog : Prog ) : Int = {
    prog.weight.replace("(", "" ).replace( ")", "" ).toInt
  }
  
  
  def calcSum( prog : Prog ) : Int = {
    
    if( prog.sum == 0 ){
      
      val childSums = prog.children.map( calcSum( _ ) )
      val sum = childSums.foldLeft(0)( (a:Int,b:Int) => { a + b } )
      prog.sum = weightToInt( prog ) + sum
  ...
  def findUnbalanced( prog : Prog ) : Prog = {
    
    // are all the children balanced?
    val cmap = prog.children.groupBy( _.sum )
    if( cmap.keys.size > 1 ){
      // children are unbalanced
      // get the unbalanced child
      val clists = cmap.values.toList.filter( _.size == 1 )
      findUnbalanced( clists.head.head )
    }
    else {
      prog
    }
