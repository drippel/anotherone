...
3 8 6 5"""
  ...
  val part1 = """104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292
...
1763	464	182	1932	1209	640	545	931	1979	197	1774	174	2074	1800	939	161"""
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    val lines = toLines(part1)
    val nums = lines.map( ( l : String) => { filterNums( toNums( l ) ) } )
    
    val check = nums.foldLeft(0)( (a:Int,b:Int) => { a + b } )
    Console.println(check)
    
  }
  
  def toLines( raw : String ) : List[String] = {
    val ls = raw.split('\n')
    val lines = ls.map( (s:String) => { s.trim() } )
    lines.toList
  }
  
  def toNums( line : String ) : List[Int] = {
    val is = line.split( '\t' ) 
    val ii = is.map( ( s : String ) => { s.toInt } )
    ii.toList.sorted
  }
  
  def filterNums( is : List[Int] ) : Int = {
    
    // take the head
    def inner( i : Int, ii : List[Int], accum : List[Int] ) : List[Int] = {
      ...
      if( !accum.isEmpty ){
        ...
        val div = ii.find( ( a : Int ) => { a % i == 0 } )
        
        div match {
          case Some(d) => { inner( ii.head, ii.tail, List(i,d) ) }
          case None => { inner( ii.head, ii.tail, List() ) }
        }
    ...
    val res = inner( is.head, is.tail, List() )
    
    res.last / res.head
    
  }
  
  def toCheck( is : List[Int] ) : Int = {
    is.last - is.head
  }
}