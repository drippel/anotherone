...
p=<749,-3446,-111>, v=<114,-490,-13>, a=<-9,35,1>"""
  ...
  def convert( lines : List[String] ) : List[Particle] = {
    
    val ps = ListBuffer[Particle]()
  ...
  def convert( src : String ) : Coord = {
    
    val s = src.indexOf('<') + 1
    val e = src.indexOf('>')
    
    val v = src.substring(s, e )
    
    val ps = v.split(",")
    
    new Coord( ps(0).trim().toInt, ps(1).trim().toInt, ps(2).trim().toInt )
