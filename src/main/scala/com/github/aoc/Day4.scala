...
object Day4 {
  ...
  val data = """oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
qrwu mgnw hvflf ytspp mco ikvbqg fflvh wts cbbf"""
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    val words = lines.map( Common.toWords( _ ) )
    val valid = words.filter( isValid2( _ ) )
    Console.println( valid.size )
  }
  
  def isValid( line : List[String] ) : Boolean = {
    
    val c1 = line.size
    val c2 = line.toSet.size
    
    c1 == c2
    
  }
  
  def isValid2( line : List[String] ) : Boolean = {
    
    val c1 = line.size
    
    val words = line.map( ( s : String ) => { s.sorted } )
    
    val c2 = words.toSet.size
